 ...
 ******************************************************************************/
 ...
 void IO_EXTENSION_Init()
 {
     ...
     // Drive the expander pins to a known-good board state on boot.
     uint8_t data[2] = {IO_EXTENSION_IO_OUTPUT_ADDR, IO_EXTENSION.Last_io_value};
     DEV_I2C_Write_Nbyte(IO_EXTENSION.addr, data, 2);
     IO_EXTENSION_Output(IO_EXTENSION_IO_5, 0); // Select USB instead of CAN
     IO_EXTENSION_Output(IO_EXTENSION_IO_3, 1); // Release LCD reset
     IO_EXTENSION_Output(IO_EXTENSION_IO_1, 1); // Release touch reset
     IO_EXTENSION_Output(IO_EXTENSION_IO_2, 0); // Keep backlight off until panel init completes
 }
