...
esp_lcd_touch_handle_t touch_gt911_init()
{
    ...
    DEV_Digital_Write(EXAMPLE_PIN_NUM_TOUCH_INT, 0);  // Set interrupt pin low
    IO_EXTENSION_Output(IO_EXTENSION_IO_0, 0);  // Also set expander-based interrupt pin low (IO0)

    vTaskDelay(pdMS_TO_TICKS(100));  // Wait for another 100ms
    IO_EXTENSION_Output(IO_EXTENSION_IO_1, 1);  // Release reset (IO1)
    
    vTaskDelay(pdMS_TO_TICKS(500));  // Wait for 500ms to ensure the touch controller is ready
